...
export default withOktaAuth(class SecureTrashedDraftsList extends Component {
    _isMounted = false;
    _loaded = false;
    ...
    componentDidUpdate() {
        ...
        this.checkUser();
        this.loadDrafts();
        console.log('Fuckin update');
    }

    loadDrafts() {
        if(!this._loaded && this.state.user) {
            console.log('Loading ' + this.state.user); 
            axios.get(`${ConfigDetails().backend_uri}drafts/bin/${this.state.user}`)
                .then(response => {
